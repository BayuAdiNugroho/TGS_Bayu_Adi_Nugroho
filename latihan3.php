...
<?php
        ...
        $query=mysqli_query($con,"SELECT * FROM tabelmhs WHERE npm='$npm'")or die(mysql_error());
        while($data=mysqli_fetch_array($query)){

        if(isset($_POST['proses']))
        {
        $nama= $_POST['nama'];
        $query=mysqli_query($con,"UPDATE tabelmhs SET npm='$npm', nama='$nama' WHERE npm='$npm'");
